...
ALGORITHM (Step-by-step):
	 1. IMDB is an in-built Keras datset, Keras allows to load the dataset already stored in tensorflow in 
		a format that is ready for use in neural network and deep learning models.We use imbd.load_data() function
		tp load the dataset in our program and system.
    2. Now when we view there reviews using python we see that reviews have been preprocessed in keras, 
	     and each review is encoded as a sequence of word indexes 
    ...
    3.	We are only interested in the first 5,000 most used words in the dataset. Therefore we filtered with 5000 words.
    4.  After loading data we find that the average review has just under 300 words with a standard deviation of just over 200 words. 
	    Hence we choose to cap the maximum review length at 500 words, truncating reviews longer than that and padding reviews shorter
		than that with 0 values.So that tenserflow is able to craete the matrix for our data.
    5.  Then we use a technique called word embedding where words are encoded as real-valued vectors in a high-dimensional space, 
    ...
    6. The output of this first layer would be a matrix with the size 50×500 for a given review training or test pattern in integer format.
    7. Embedding layer as the input layer, setting the vocabulary to 5,000,the word vector size to 50 dimensions and the input_length to 500.
    8. For our single layer model we flatten the Embedded layers output to one dimension, then use one dense hidden layer of 200 units. 
	   The output layer has one neuron and will use a sigmoid activation to output values of 0 and 1 as predictions. 
    9.  After training is done we evaluvate the results on test data and summerize the results.	   
    10. Then finally we use convolutional neural netwoks to as a training model for learning to recognize 
	    data here also we latten the Embedded layers output to one dimension, then use one dense hidden layer of 200 units. 
	   The output layer has one neuron and will use a sigmoid activation to output values of 0 and 1 as predictions.  
    11.  After training is done we evaluvate the results on test data and summerize the results.	
